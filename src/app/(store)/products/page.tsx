...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  if (size) where.variants = { some: { size: { in: size.split(',') }, stock: { gt: 0 } } };
  if (color) where.variants = { some: { color: { in: color.split(',') } } };
  if (gender) where.gender = { in: gender.split(',').map((g: string) => g.toUpperCase()) };
  ...
  if (price) {
    ...
    if (!isNaN(max)) where.basePrice = { ...(where.basePrice as Record<string, unknown> ?? {}), lte: max };
  }
  ...
  const categoryOptions = categories.map((c: { name: string; slug: string; _count: { products: number } }) => ({
    label: c.name, value: c.slug, count: c._count.products,
  }));
  const brandOptions = brands.map((b: { name: string; slug: string; _count: { products: number } }) => ({
    label: b.name, value: b.slug, count: b._count.products,
  }));
  const priceRanges = [
