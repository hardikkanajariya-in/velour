...
export default function AdminBlogPage() {
    ...
    },
    {
      ...
      render: (post: BlogPost) => (
        <div className="flex gap-2 justify-end">
          <button onClick={() => openEdit(post)} className="p-1.5 hover:bg-muted rounded-md transition-colors">
            <Pencil className="w-4 h-4" />
          </button>
          <button onClick={() => handleDelete(post.id)} className="p-1.5 hover:bg-red-50 text-red-500 rounded-md transition-colors">
  ...
  if (loading) return <div className="animate-pulse h-64 bg-muted rounded-card" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingPost ? 'Edit Post' : 'New Post'}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Author" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="Admin" />
            <Select
              label="Category"
              value={category}
          ...
          <Input label="Tags (comma-separated)" value={tags} onChange={(e) => setTags(e.target.value)} placeholder="fashion, summer, trends" />
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={isPublished} onChange={(e) => setIsPublished(e.target.checked)} className="accent-accent" />
            Publish immediately
          </label>
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="ghost" onClick={() => setShowModal(false)}>Cancel</Button>
            <Button variant="primary" onClick={handleSubmit}>{editingPost ? 'Update' : 'Create'}</Button>
          </div>
        </div>
      </Modal>
    </div>
