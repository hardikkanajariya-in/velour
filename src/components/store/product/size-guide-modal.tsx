...
export function SizeGuideModal({ isOpen, onClose, gender = 'UNISEX' }: SizeGuideModalProps) {
  const chart = gender === 'WOMEN' ? womenSizeChart : menSizeChart;
  const headers = gender === 'WOMEN'
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b">
                {headers.map((h) => (
                  <th key={h} className="py-2.5 px-3 text-left font-semibold text-xs uppercase tracking-wide">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {chart.map((row) => (
                <tr key={row.size} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                  ...
                  <td className="py-2.5 px-3 text-muted-foreground">{row.hip}</td>
                </tr>
              ))}
            </tbody>
          </table>
